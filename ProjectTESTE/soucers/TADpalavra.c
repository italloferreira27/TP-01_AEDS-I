...
void InserirElemLP(ListaPala *LP, TPalavra item, char *charptr, Tlista* Lista_linha ,Titem item_linha){
    ...
    if(result == 0){
        ...
        if(LP->Ulitmo != NULL){ //espaco esta disponivel
            ...
            LP->Ulitmo->prox = NULL;
            LL_Inserir(Lista_linha, item_linha);
        }
        LP->nroElem++;
    }
...
void ImprimirLP(ListaPala *LP, Tlista* Lista_linha){
    Apontador aux;
    ...
    while (aux != NULL){
        printf("-----------------------\n");
        printf("Palavra: %s\n",aux->palavra.item);
        //LL_Imprimir(Lista_linha); //Função imprimir linhas
        //printf("Linhas: \n");
        printf("-----------------------\n");
        aux = aux->prox;
    }
