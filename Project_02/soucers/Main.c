...
int main(){
    ...
    //char palavra[50]; //(fscanf)
    char *palavra; //Lê cada palavra da linha (fgets)
    int cont = 1;
    char z[1];
    char l;
    ...
    while(escolha != 9){
        switch(escolha){
                ...
                if(TXTptr == NULL){
                    ...
                    while(fgets(linha_texto, 100, TXTptr) != NULL){
                        ...
                        while(palavra){
                            let.letra = palavra[0];
                            item.chave = cont;
                            verifica_letra(&letras, let, palavra, obj, &linha, item);
            ...
            case 8:
                ...
                scanf("%s",z);
                l = z[0];
                imprimeletradada(&letras, l, &linha);
                break;
            default:
